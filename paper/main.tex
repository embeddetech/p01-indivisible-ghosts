...
is exactly the condition for a probability-conserving indivisible process to
exist, but the metric-corrected process is the ordinary process of the
...
$-(\sin\theta+\tilde\kappa)$---while information backflow vanishes: the two
...
analysis proves an ``explosion dictionary'': one classical integral decides
...
Pais--Uhlenbeck operator we further prove an exact channelization whose
sign prediction the numerics confirm ($\lambda>0$: limit-circle channels;
...
one-dimensional fiber non-self-adjoint by a glued-tail von Neumann
certificate---no compactly supported function can witness
deficiency---and reduce the remaining $M\to\infty$ step to Gaussian-beam
assembly along numerically characterized escape rays. The ghost problem,
viewed in the
...
conversation and project briefing are also preserved.
\end{quote}
...
\cite{DivisibleIndivisible2025} remains entirely within Hermitian maps).
...
\item \textbf{The broken phase is a valid open process; the two
  ...
  is recurrent instead. Information backflow, by contrast, vanishes in the
...
\item \textbf{The explosion dictionary} (Sec.~\ref{sec:dictionary}).
  The \emph{explosion dictionary} (Theorem~\ref{thm:dictionary}): a
  single classical integral---the time of
  flight to infinity---decides classical completeness, quantum essential
  self-adjointness (via the identity ``deficiency norm $=$ time of
...
\item \textbf{Multidimensional channel theory}
  ...
  allows: an exact channelization with a confirmed sign prediction
  ...
  Maslov feasibility study reducing the remaining $M\to\infty$ step to
  Gaussian-beam assembly along the escape rays.
\end{enumerate}
...
\label{eq:dictionary}
\end{equation}
with $\odot$ the entrywise product; $|U|^{2}$ is doubly stochastic iff $U$
is unitary, and indivisibility expresses interference,
...
while the corrected process is
...
$H_{12}H_{21}>0$. The dimer \eqref{eq:dimer} fails (K1) for every
...
is the cycle affinity of the rate graph: it vanishes iff the classical
...
no fixed beables exist---indeed the periodic-boundary spectrum is a
complex loop (the spectral winding that classifies the skin effect
\cite{Okuma2020}), so no closed process exists at all. An open chain is a
...
Thus the broken phase is \emph{backflow-Markovian yet Barandes-indivisible}:
the two notions genuinely dissociate, indivisibility being strictly finer.
That divisibility- and information-flow-based notions of Markovianity are
inequivalent in general is known for quantum channels
\cite{ChruscinskiRS2018}; the $\PT$ dimer makes the dissociation exact,
...
is a shifted $\PT$ dimer, so ``overdamped open dynamics'' and ``broken
$\PT$'' coincide.)
...
\end{equation}
both matched numerically to $\sim$4 decimals (Fig.~\ref{fig:cascade};
...
numerics), while the normal-sign twin (squares) stays bounded near
$10^{-3}$---the instability threshold collapses from the sum to the
difference frequency. Right: escape from the occupation ball $n_1\le10$
...
weight, while the free control never passes $r\approx6$.
...
Complementary operator-theoretic stability results for interacting
ghosts---conserved quantities with discrete spectrum \cite{Deffayet2026}
and exact moment bounds \cite{EwasiukProfumo2026}---identify interaction
structures in which the cascade is arrested; in the present language,
those are mechanisms enforcing recurrence.
...
($V\to-\infty$), and define the classical time of flight
...
limit circle \cite{ReedSimon2}, by the identity
\begin{equation}
|\psi_{\pm}(x)|^{2}\;\approx\;\bigl[2(E-V(x))\bigr]^{-1/2}
\quad\Longrightarrow\quad
\int^{\infty}\!|\psi_{\pm}|^{2}\,\dd x \;=\; T :
\label{eq:onelineidentity}
\end{equation}
\emph{the deficiency solution's norm is the classical time of flight}.
Consequently:
\begin{itemize}
...
with local spacing $\Delta E=\pi/T_{\rm cross}(E)$, $T_{\rm cross}$ the
...
$V=-x^{2}$): the spacing law matches to $1$--$3.5\%$ in all eight
$(V,L)$ cases tested and to $0.3\%$ at the largest box
(Table~\ref{tab:spacing}); the eigenvalue nearest $E=10$ sweeps
...
super-quadratically falling (limit circle) and $h_{\perp}$ self-adjoint
...
$L^{2}$, since the limit-circle property is independent of the spectral
...
$B=\mu f(x_1)g(x_2)$ with $f,g$ bounded. Then $H_0+B$ has the same
...
coupling leaves the counts unchanged (Kato stability made visible)---and a
...
\begin{proposition}[exact channelization of the quartic PU]
\label{prop:bo}
...
\qquad
T(p)=\lambda p^{4}-\frac{\gamma\bar\omega^{2}}{2}p^{2},
\end{equation}
with $\bar\omega^2=(\omega_1\omega_2)^2$ (verified in Fock space via the
...
$\hat p^{2}/(2\gamma\Omega)-\lambda k^{4}+(\gamma\bar\omega^{2}/2)k^{2}$;
...
baseline (ratios $0.17$--$0.46$ at two basis scales, none growing with
truncation), separated from the $\lambda>0$ limit-circle signature
(ratios up to $3.8$, persistently $O(1)$ and \emph{largest at the finest
truncation} $N=64$; a single early window at one basis scale dips to
$0.34$, but the trend never decays), while the $\lambda<0$ ghost pair
remains at baseline up to $N=64$. The $\lambda<0$ quartic PU thus joins the $\lambda<0$ ghost pair
...
\]
with $P_{2,M}$ the truncated momentum of mode 2, has deficiency indices
$(2M,2M)$.
\end{theorem}
...
the 1D fiber operator is \emph{certified} not essentially self-adjoint.
...
growing branch is polynomial, $k^{+Z/1.12}$, non-$L^{2}$ precisely for
$Z>1.12$. On the full two-dimensional operator, windowed candidates
supply a \emph{closure-defect diagnostic}: falling-channel operators hug
the forbidden bound (ratios $1.21$--$1.29$ for all five $\lambda>0$ beam
launches, indistinguishable from the 1D limit-circle control at $1.22$),
...
$s''''+\gamma\Omega s''+\gamma\bar\omega^{2}s=4\lambda s^{3}$, so the
...
\begin{center}
  \url{https://github.com/embeddetech/indivisible-ghosts}
\end{center}
...
(NumPy/SciPy), each of which runs in seconds to minutes on a laptop:
...
A.~Mostafazadeh,
\emph{Pseudo-Hermiticity versus $\PT$ symmetry},
